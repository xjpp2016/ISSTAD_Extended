...
class Aug_old:
    def __init__(self, min_pixel=8):
        self.min_pixel = min_pixel

    def get_random_patch_from_img(self, img):
        w, h = img.size
        min_w, max_w = self.min_pixel, (w - self.min_pixel) / 8 + self.min_pixel
        min_h, max_h = self.min_pixel, (h - self.min_pixel) / 8 + self.min_pixel
    ...
    def get_change(self):
        ...
        if num == 0:
            transform_list += [transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=1)]
        elif num == 1:
            transform_list += [transforms.RandomGrayscale(p=1)]
        elif num == 2:
            transform_list += [transforms.RandomApply([GaussianBlur([.1, 2.])], p=1)]
        elif num == 3:
            transform_list += [
            ...
            transform_list += [transforms.RandomVerticalFlip(p=1)]
        elif num == 6:
            transform_list += [
                transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=1),
                transforms.RandomGrayscale(p=1),
    ...
    def __call__(self, image, label):
        ...
        while len(boxes) < num:
            ...
            patch, x, y, sw, sh = self.get_random_patch_from_img(image)
            boxes.append([x, y, x + sw, y + sh])
            patch_ = change(patch)
            if (np.array(patch) - np.array(patch_)).mean() != 0:
                patch_label = Image.new('L', (sw, sh), 255)
                image.paste(patch_, (int(x), int(y)))
                label.paste(patch_label, (int(x), int(y)))
        return image, label